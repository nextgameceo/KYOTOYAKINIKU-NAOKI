  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-stretch h-14 bg-[#0a0a0a]/95 backdrop-blur-sm border-b border-white/10">
      <div className="flex items-center flex-1 px-4 gap-6">
        <Link href="/" className="font-['Zen_Old_Mincho',serif] text-[#c8a84a] text-lg font-black tracking-widest">
          なおき
        </Link>
        <ul className="hidden md:flex gap-1 h-full">
          {links.map(l => (
            <li key={l.href} className="h-full">
              <Link
                href={l.href}
                className="flex items-center h-full px-4 text-xs tracking-widest text-white/70 hover:text-white border-b-2 border-transparent hover:border-[#b01020] transition-all"
              >
                {l.label}
              </Link>
            </li>
      ...
      <div className="flex items-stretch">
        <a href="tel:052-990-6329" className="hidden md:flex flex-col justify-center px-4 text-white bg-white/5 border-l border-white/10">
        ...
        <Link href="/reserve" className="flex items-center justify-center bg-[#b01020] hover:bg-[#d01828] text-white text-xs tracking-widest px-5 transition-colors">
          WEB予約
        </Link>
        <button
      ...
      {open && (
        <div className="absolute top-14 left-0 right-0 bg-[#0a0a0a] border-b border-white/10 md:hidden">
          {links.map(l => (
            <Link
              key={l.href}
              href={l.href}

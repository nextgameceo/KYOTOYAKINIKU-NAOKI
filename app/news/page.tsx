...
export default async function NewsListPage() {
  try {
    const data = await client.get({ 
      endpoint: 'news', // microCMSのエンドポイント名を確認してください
      queries: { limit: 100 },
      ...
      <main className="min-h-screen bg-black text-[#e0d8c3] font-serif py-24 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-24 border-b border-white/5 pb-12">
            ...
            <h1 className="text-4xl md:text-5xl tracking-[0.4em] mb-4 text-white uppercase">最新情報</h1>
            <p className="text-[#d4af37] text-[10px] tracking-[0.5em] uppercase font-sans font-bold">News & Topics</p>
          </div>
            ...
            {data.contents.map((post: any) => (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center gap-6 md:gap-12">
                  <span className="text-[10px] tracking-[0.3em] font-sans font-bold text-zinc-600 group-hover:text-[#d4af37] transition-colors">
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center font-serif">
        <p className="tracking-widest opacity-40 italic text-center leading-loose">
          ー お知らせの準備中です ー<br/>
          <span className="text-[10px] mt-4 block">microCMSのエンドポイント「news」を確認してください</span>
        </p>
      </div>
    );
  }
